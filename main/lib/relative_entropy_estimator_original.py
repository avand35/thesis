...
def convert_to_array(to_convert: str, cast_to_float = False) -> np.array:
    if to_convert == "NaN":
        return np.asarray([])
    if '"' in to_convert:
        inside_value = False
        ...
        if not cast_to_float:
            return np.asarray(clean_string.split(","))
        else:
            return np.asarray([float(x) for x in clean_string.split(",")])
...
class DB_stats:
    ...
    def inverse_dist(self, inverse_mf: Callable[[Any], list], generalization_degree = -1):
        ...
        if generalization_degree < 1:
            self.generalization_degree = float(len(new_vals))/float(self.most_common_vals.size)
        else:
