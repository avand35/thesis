...
BEGIN
    ...
    RETURN '[' || lower_bound::TEXT || ',' || upper_bound::TEXT || ')';
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION blur(input TEXT, n INTEGER)
RETURNS TEXT
AS $$
def blur(input, n):
    input_str = str(input)
    if n >= len(input_str):
        return "X" * len(input_str)
    else:
        return input_str[:-n] + ("X" * n)
return blur(input,n)
$$ LANGUAGE plpython3u;


CREATE OR REPLACE FUNCTION add_noise(mean FLOAT, std_dev FLOAT, val FLOAT, cast_it INT)
RETURNS NUMERIC
AS $$
import random

def add_noise(mean, std_dev, val,cast_it):
    ...
    if cast_it==1:
        ...
        return result
return add_noise(mean,std_dev,val,cast_it)
$$ LANGUAGE plpython3u;

CREATE OR REPLACE FUNCTION generalize_category(input_dict JSONB, input_string TEXT)
RETURNS TEXT
AS $$
import json

def generalize_category(input_dict, input_string):
    try:
        dict_data = json.loads(input_dict)
        result = dict_data[input_string]
        return result
    except KeyError:
        return input_string
return generalize_category(input_dict,input_string)
$$ LANGUAGE plpython3u;
...
input_date2 = date(year, month, day)
if level == 'DAYS':
    min_date = input_date2 - timedelta(days=noise2)
    max_date = input_date2 + timedelta(days=noise2)
elif level == 'WEEKS':
    min_date = input_date2 - timedelta(weeks=noise2)
    max_date = input_date2 + timedelta(weeks=noise2)
elif level == 'MONTHS':
    min_date = input_date2 - timedelta(days=noise2*30)
    max_date = input_date2 + timedelta(days=noise2*30)
elif level == 'YEARS':
    min_date = input_date2.replace(year=input_date2.year - noise2)
    max_date = input_date2.replace(year=input_date2.year + noise2)
else:
    raise Exception("For the MF NOISE_DATE level should be either 'DAYS', 'WEEKS', 'MONTHS', or 'YEARS' and was " + level)
